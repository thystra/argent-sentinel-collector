#!/usr/bin/env python3
# Argent Sentinel v0.5.0.4 packaging regression tests.
...
class V046Test(unittest.TestCase):
    def test_release_versions_are_consistent(self) -> None:
        ...
        for relative in (
            ...
            self.assertIn('APP_VERSION = "0.5.0.4"', source)

        builder = (ROOT / "packaging/build_debs.py").read_text()
        self.assertIn('if upstream != "0.5.0.4":', builder)
    ...
    def test_server_package_root_contains_timer_assets(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            metadata = builder.make_server(root, "0.5.0.4-1")
