#!/usr/bin/env python3
# Argent Sentinel v0.4.5 release regression tests.
...
class V044Test(unittest.TestCase):
    def test_release_versions_and_banner_layout(self) -> None:
        self.assertEqual("0.4.5", (ROOT / "VERSION").read_text().strip())
        ...
        for relative in (
            ...
            self.assertIn('APP_VERSION = "0.4.5"', source)

        builder = (ROOT / "packaging/build_debs.py").read_text()
        self.assertIn('if upstream != "0.4.5":', builder)
